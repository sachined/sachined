...
*   **The Architecture**: A Directed Acyclic Graph (DAG) orchestrated via LangGraph, featuring parallel execution and conditional routing (e.g., skipping dividend analysis for non-dividend stocks to save costs).
*   **Security Layer**: Implemented Semnatic Guardrails that use a dedicated Security Agent to sanitize inputs and block prompt injections before any high-cost API calls are made.
...
*   🛡️ **Security Focus**: Developing advanced "Prompt Shield" architectures to protect internal agent instructions from being leaked via indirect injections.

---

### 🧠 Summary of Learning: The FinSurf Journey

Building FinSurf was an exercise in bridging cutting-edge AI orchestration with professional-grade document engineering.
...
*   **Containerization**: Integrated Docker to ensure environment parity across local development and cloud deployment.
*   **Telemetry**: Built a SQLite-backed cost-tracking system to monitor per-agent token usage and enforce daily budget caps.
